...
class VideoDataset_train(data.Dataset):
    ...
    def __init__(self, dataset_ir, dataset_rgb, seq_len=12, sample='evenly', transform=None, index1=[], index2=[],
        ...
        # print(self.dataset_ir)
        #
        # self.imgIndex_rgb = [np.random.choice(len(dataset_rgb[i][0])) for i in range(self.dataset_rgb)]
        self.imgIndex_ir = [np.random.choice(len(i[0])) for i in self.dataset_ir]
        self.imgIndex_rgb = [np.random.choice(len(i[0])) for i in self.dataset_rgb]

        # 修改：使用集合来跟踪已经保存的图像路径，避免重复
        self.seen_thermal_paths = set()
        self.seen_rgb_paths = set()
        self.train_color_image = []  # 存储所有RGB图像
        self.train_thermal_image = []  # 存储所有IR图像
        self.train_color_labels = []  # 存储所有RGB图像
        self.train_thermal_labels = []  # 存储所有IR图像
    ...
    def __getitem__(self, index):
        if not self.isRandom:  # 每次从tracklet中抽取的图片固定
            ...
            # 获取图像，id信息和这个id下面的所有的图像路径
            img_rgb_paths, pid_rgb, camid_rgb = self.dataset_rgb[self.cIndex[index]]
            num_rgb = len(img_rgb_paths)
            # print(pid_rgb, img_rgb_paths)
            S = self.seq_len
            sample_clip_ir = []
            ...
            if self.sample == 'random':
                ...
                for index in number_ir:
                    ...
                    imgs_ir.append(img)
                    # 修改：检查是否已经保存过这个图像
                    if img_path not in self.seen_thermal_paths:
                        flag = 1
                        self.train_thermal_image.append(img)
                        self.train_thermal_labels.append(pid_ir)
                        self.seen_thermal_paths.add(img_path)
                        path = img_path
                imgs_ir = torch.cat(imgs_ir, dim=0)
                ...
                # print(idx2, idx1)
                imgs_rgb = []
                for index in number_rgb:  # 这里原始是rgb
                    index = int(index)
                    img_path = img_rgb_paths[index]
                    img = read_image(img_path)
                    ...
                    '只在训练的时候存储'
                    # 修改：检查是否已经保存过这个图像
                    if img_path not in self.seen_rgb_paths:
                        if flag:
                            self.train_color_image.append(img)
                            self.train_color_labels.append(pid_rgb)
                            self.seen_rgb_paths.add(img_path)
                        else:
                            self.train_thermal_image.pop()
                            self.train_thermal_labels.pop()
                            if path is not None:
                                self.seen_thermal_paths.remove(path)
                imgs_rgb = torch.cat(imgs_rgb, dim=0)
                # print(len(self.train_color_labels), len(self.train_thermal_labels))
