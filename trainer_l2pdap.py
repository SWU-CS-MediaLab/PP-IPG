...
class Trainer(object):
    def __init__(self, model):
        ...
        self.model = model  # 当前一个权重没有被冻结的模型
    ...
    def train(self, epoch, data_loader_train, data_loader_replay, optimizer, training_phase,
              add_num=0, old_model=None, replay=False,writer = None, freere_backbone = False):
        # old_model是上一个任务，全部被冻结的  用于kd损失函数

        if freere_backbone:
            self.model.freeze_backbone(self.model.sinet, training_phase - 1)
        else:
            self.model.train()
            start_layer = 5
            for name, param in self.model.named_parameters():
                ...
                if (training_phase == 2 or training_phase == 3 or training_phase == 4) and ('adptList' in name and 'dap_downsample' in name):
        ...
        for batch_idx, (input1, input2, label1, label2) in enumerate(data_loader_train):
            ...
            if replay is True:
                ...
                if self.model.KeyToken:
                    ...
                    # 这里把正确的任务id数量挑出来
                    # print(out_old["old_logits"][domain-1],out_r["old_logits"][domain-1])
                    loss_kd_rehersal = self.loss_kd_js(out_old["old_logits"][domain-1],out_r["old_logits"][domain-1])

                    loss_tp_r = self.criterion_triple(out_r["old_feats"][domain-1],labels_r)[0]

                    loss_rehersal = (1-lamba) * loss_tp_r + lamba * loss_kd_rehersal# * 5
