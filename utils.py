...
def get_data(dataset, args, transform_train, transform_test):
    if dataset == "RegDB":  # query是可见光 query是红外
    ...
    elif dataset == "SYSU-MM01":  # query是红外光 gall是可见
    ...
    elif dataset == "LLCM":  # query是可见光 gall是红外
    ...
    elif dataset == "VCM":  # query是红外光 gall是可见
        processed_data = VCM()
        # image-based method
        trainset = VideoDataset_train(processed_data.train_ir, processed_data.train_rgb, 1, 'video_train',
                                      transform_train)
        trainset.train_color_label = processed_data.rgb_label
        trainset.train_thermal_label = processed_data.ir_label
        # print(trainset.train_color_label, trainset.train_thermal_label)
        # trainset.train_color_image = processed_data.train_rgb
        # trainset.train_thermal_image = processed_data.train_ir

        initset = VideoDataset_train(processed_data.train_ir, processed_data.train_rgb, 1, "video_train",
                                     transform_test)
        if args.mode_vcm == 1:
            gallset = VideoDataset_test(processed_data.gallery, 1, "video_test", transform_test,
                                        processed_data.gallary_cam)
            queryset = VideoDataset_test(processed_data.query, 1, "video_test", transform_test,
                                         processed_data.query_cam)
            gallset.test_label = processed_data.gallery_labels
            queryset.test_label = processed_data.query_labels
        else:
            gallset = VideoDataset_test(processed_data.gallery_1, 1, "video_test", transform_test,
                                        processed_data.gallery_cam1)
            queryset = VideoDataset_test(processed_data.query_1, 1, "video_test", transform_test,
                                         processed_data.query_cam1)
    ...
    init_loader = data.DataLoader(initset, batch_size=args.test_batch, shuffle=False, sampler=sequentialSampler,
...
def extract_features(model, data_loader, training_phase=1):
    ...
    with torch.no_grad():
        for i, (input1, input2, labels1, labels2) in enumerate(data_loader):
            ...
            features = model(input1, input2, training_phase=training_phase, modal = 3)["test"]
...
def select_replay_samples(model, dataset, dataloader, training_phase=1, add_num=0, old_datas=None,
    ...
    # if hasattr(dataset, 'train_color_labels'):
    #     print("create replay datasets for vcm")
    #     color_imgs, thermal_imgs, color_labels, thermal_labels = dataset.train_color_image, dataset.train_thermal_image, dataset.train_color_labels, dataset.train_thermal_labels
    #     print(len(color_labels), len(thermal_labels), len(color_imgs), len(thermal_imgs))
    # else:
        # 对象没有这个属性
    # print("create replay datasets for regdb/sysu/llcm")
    ...
    # print("extracting features",len(features_visible), len(features_thermal))
    ...
    class_centers_visible = [torch.stack(pid2visible_features[pid]).mean(0) for pid in
                             sorted(pid2visible_features.keys())]
    class_centers_visible = F.normalize(torch.stack(class_centers_visible), dim=1)
    class_centers_thermal = [torch.stack(pid2thermal_features[pid]).mean(0) for pid in
                             sorted(pid2thermal_features.keys())]
    ...
    for pid in select_pids:
        ...
        for color_idx, thermal_idx in zip(visible_idxs, thermal_idxs):
            ...
            replay_data.append(
                (pid2color_imgs[pid][color_idx], pid2thermal_imgs[pid][thermal_idx], pid + add_num, training_phase))
...
class IdentitySampler_accelerate(Sampler):
    """Sample person identities evenly in each batch.
        Args:
            train_color_label, train_thermal_label: labels of two modalities
            color_pos, thermal_pos: positions of each identity
    ...
    def __init__(self, train_color_label, train_thermal_label, color_pos, thermal_pos, num_pos, batchSize):
        ...
        # 进程隔离的随机生成器
        self.seed = 1
        self.rng = np.random.default_rng(self.seed + self.accelerator.process_index)
        ...
        for j in range(int(N / (batchSize * num_pos)) + 1):
            ...
            for i in range(batchSize):
                # print(batch_idx[i])
                # 确保每个进程选择不同的样本
                sample_color = self.rng.choice(color_pos[batch_idx[i]], num_pos, replace=False)
                sample_thermal = self.rng.choice(thermal_pos[batch_idx[i]], num_pos, replace=False)
