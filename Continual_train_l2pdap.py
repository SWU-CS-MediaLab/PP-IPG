...
from random_erasing import ModerateRLE, RadicalRLE, RandomErasing
from utils import *
from model import TTQK
from trainer_l2pdap import Trainer
from eval_metrics import eval_func2_AIM as eval_func2
from optimizer import adamw
from scheduler import create_scheduler
from tensorboardX import SummaryWriter

torch.multiprocessing.set_sharing_strategy('file_system')
# from model_finetune import MyModel
# from model_l2p_dap import MyModel as MyModelL2pAdpt
from model_l2p_dap import MyModel as MyModelL2pAdpt
...
def main_worker():
    ...
    # suffix = f"bs36-dap20-l2p(8,2,20)-prompt投影(增强)加残差连接-所有token传入DAP-记忆库参数修正-学习率计划修正 "
    suffix = f"交换数据集-sysu-llcm-regdb-vcm"
    ...
    datasets = ["SYSU-MM01", "LLCM","RegDB",  "VCM"]
    epochs = [args.sysu_epochs,args.llcm_epochs,args.regdb_epochs,   args.vcm_epochs]
    ...
    # net = MyModel(class_num=num_classes_regdb, embed_dim=768, KeyToken=True, GeneralToken=False,
    #              isShareBlock=False, base=False, instance_wise=True, withPrompt=True)
    #
    # net = MyModelL2p(class_num=num_classes_regdb, embed_dim=768, KeyToken=True, GeneralToken=False,
    #                  isShareBlock=False, base=False, instance_wise=False,
    #                  withPrompt=True, prompt_length=20,  # 提示的长度
    #                  pool_size=8, top_k=2)

    net = MyModelL2pAdpt(class_num=num_classes_regdb, embed_dim=768, KeyToken=True, base=False, instance_wise=False,
                     withPrompt=True, prompt_length=20,  # 提示的长度
                     pool_size=8, top_k=2, withAdpt=True)

    # net = MyModelL2pAdpt(class_num=num_classes_regdb, embed_dim=768, KeyToken=True, GeneralToken=False,
    #                      isShareBlock=False, base=False, instance_wise=False,
    #                      withPrompt=True, prompt_length=20,  # 提示的长度
    #                      pool_size=8, top_k=2, withAdpt=True)
    ...
    scheduler = create_scheduler(optimizer, stage=1)
    ...
    # net.update_id()  # add id for prompt

    trainer = Trainer(net)
    color_pos, thermal_pos = GenIdx(dataset_regdb.train_color_label, dataset_regdb.train_thermal_label)

    for epoch in range(start_epoch, epochs[0]):
        scheduler.step(epoch)
        current_lr = optimizer.param_groups[0]["lr"]
        if args.distributed:
            sampler = DistributedIdentitySampler(dataset_regdb.train_color_label, dataset_regdb.train_thermal_label,
                                                 color_pos, thermal_pos,
                                                 num_pos=args.num_pos,
                                                 batch_size=args.batch_size)  # distributed sampler
        else:
        ...
        trainer.train(epoch, train_loader_regdb, None, optimizer, 1, 0, writer=writer,
                      freere_backbone=args.freere_backbone)
    ...
    replay_dataloader, regdb_replay_dataset, pid_pos, replay_pids, pid_nums = select_replay_samples(net, dataset_regdb,
                                                                                                    init_loader_regdb,
                                                                                                    training_phase=1,
                                                                                                    select_samples=4)
    ...
    for epoch in range(start_epoch, epochs[1]):
        ...
        if args.distributed:
            sampler = DistributedIdentitySampler(dataset_sysu.train_color_label, dataset_sysu.train_thermal_label,
                                                 color_pos, thermal_pos,
                                                 num_pos=args.num_pos,
                                                 batch_size=args.batch_size)  # distributed sampler
        else:
        ...
        if epoch % 2 == 0:
            ...
            for name, test_loader in zip(datasets, test_loaders):
                ...
                all_mAP.append(mAP)
                print(name + ' Rank-1: {:.2%}| mAP: {:.2%} '.format(cmc[0], mAP))
            print("current incremental average accuracy：Rank-1: {:.3%}, mAP:{:.3%}".format(sum(all_cmc) / len(all_cmc),
        ...
        if (epoch == epochs[1] - 1):
            ...
            for i in range(len(all_cmc) - 1):
                print(
                    'Finished stag2! {} Rank-1: {:5.1%}, mAP: {:5.1%} '.format(datasets[i], all_cmc[i][0], all_mAP[i]))

            print("Stage2's incremental average accuracy：Rank-1: {:.3%}, mAP:{:.3%}".format(
                sum([cmc[0] for cmc in all_cmc]) / len(all_cmc), sum(all_mAP) / len(all_mAP)))

        writer.add_scalar("lr", current_lr, epoch)

    replay_dataloader, sysu_replay_dataset, pid_pos, replay_pids, pid_nums = select_replay_samples(net, dataset_sysu,
                                                                                                   init_loader_sysu,
                                                                                                   training_phase=2,
    ...
    for epoch in range(start_epoch, epochs[2]):
        ...
        if args.distributed:
            sampler = DistributedIdentitySampler(dataset_llcm.train_color_label, dataset_llcm.train_thermal_label,
                                                 color_pos, thermal_pos,
                                                 num_pos=args.num_pos,
                                                 batch_size=args.batch_size)  # distributed sampler
        else:
        ...
        if (epoch == epochs[2] - 1):
            ...
            print("Stage3's incremental average accuracy：Rank-1: {:.3%}, mAP:{:.3%}".format(
                sum([cmc[0] for cmc in all_cmc]) / len(all_cmc), sum(all_mAP) / len(all_mAP)))

        writer.add_scalar("lr", current_lr, epoch)

    # Select replay data of llcm
    replay_dataloader, llcm_replay_dataset, pid_pos, replay_pids, pid_nums = select_replay_samples(net, dataset_llcm,
                                                                                                   init_loader_llcm,
                                                                                                   training_phase=3,
                                                                                                   add_num=add_num,
    ...
    for epoch in range(start_epoch, epochs[3]):
        ...
        if args.distributed:
            sampler = DistributedIdentitySampler(dataset_vcm.train_color_label, dataset_vcm.train_thermal_label,
                                                 color_pos, thermal_pos,
                                                 num_pos=args.num_pos,
                                                 batch_size=args.batch_size)  # distributed sampler
        else:
        ...
        if (epoch == epochs[3] - 1):
            ...
            for i in range(len(all_cmc) - 1):
                print(
                    'Finished stag4! {} Rank-1: {:5.1%}, mAP: {:5.1%} '.format(datasets[i], all_cmc[i][0], all_mAP[i]))

            print("Stage4's incremental average accuracy：Rank-1: {:.3%}, mAP:{:.3%}".format(
                sum([cmc[0] for cmc in all_cmc]) / len(all_cmc), sum(all_mAP) / len(all_mAP)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_h', type=int, default=256, help="input height")  # 256
    parser.add_argument('--img_w', type=int, default=128, help="input width")  # 128
    parser.add_argument('--num_pos', type=int, default=3)  # 3
    parser.add_argument('--batch-size', type=int, default=6)  # 6
    parser.add_argument('--test-batch', default=4, type=int,  # 36
    ...
    # Adam
    parser.add_argument('--base_lr', type=int, default=5e-4)  # 5e-4
    ...
    parser.add_argument('--regdb_epochs', type=int, default=20)
    parser.add_argument('--sysu_epochs', type=int, default=10)
    parser.add_argument('--llcm_epochs', type=int, default=10)
    parser.add_argument('--vcm_epochs', type=int, default=30)
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--print-freq', type=int, default=200)
    parser.add_argument('--margin', type=float, default=0.3, help='margin for the triplet loss with batch hard')
    parser.add_argument('--freere_backbone', type=bool, default=False)  # freeze backbone with prompt
    ...
    # prompt
    parser.add_argument('--prompt_lenght', type=int, default=10)
    parser.add_argument('--total_sessions', type=int, default=4)
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs_NewModel'))
    parser.add_argument('--save-model', type=str, default=osp.join('/mnt/sda1/lzx/model_data', 'save_model_NewModel'))
