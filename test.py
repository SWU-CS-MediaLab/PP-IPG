...
from utils import *
from model_l2p_dap_accelerate import MyModel as MyModelL2pAdpt
...
def main_worker():
    ...
    model_checkpoint = "/mnt/sda1/lzx/model_data/save_model_dap/测试accelerate_测试损失MSEL/last_stage_checkpoint_step_1_accelerate"
    # # net = load_checkpoint_and_dispatch(
    # #     net, checkpoint=model_checkpoint, device_map="auto"
    # # )
    #
    from accelerate import Accelerator
    accelerator = Accelerator()
    net = accelerator.unwrap_model(net)
    path_to_checkpoint = os.path.join(model_checkpoint, "pytorch_model.bin")
    ...
    # checkpoint = torch.load('/mnt/sda1/lzx/model_data/save_model_dap/测试accelerate/working_checkpoint_step_1.pth.tar')
    # # net = accelerator.unwrap_model(net)
    ...
    for name, test_loader in zip(datasets, test_loaders):
        ...
        cmc, mAP = eval_func2(net, name, test_loader, net.feat_dim, 1)  # 只测试regdb数据集
        all_cmc.append(cmc)
        all_mAP.append(mAP)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default=osp.join('/mnt/sda1/xyt/datasets/', ''))  # osp.join('/root/autodl-tmp/', '')
